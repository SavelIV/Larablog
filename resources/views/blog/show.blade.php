...
@section('content')
    <main class="blog-post">
        <div class="container">
            ...
            <p class="edica-blog-post-meta" data-aos="fade-up" data-aos-delay="200">{{ $data }} • Category: {{ $post->category->title }} • {{ $post->comments->count() }} Comments • {{ $post->likedUsers->count() }} Likes</p>
            <section class="blog-post-featured-img" data-aos="fade-up" data-aos-delay="300">
                <img src="{{ url('storage/' . $post->preview_image) }}" alt="featured image of post#{{ $post->id }}" class="img-fluid">
            </section>
                                    ...
                                    <a href="{{ route('blog.show', $post->id) }}" class="blog-post-permalink">
                                        <div class="blog-post-thumbnail-wrapper">
                                            <img src="{{ url('storage/' . $post->main_image) }}"
                                                 alt="blog post . {{ $post->id }}" class="post-thumbnail">
                                        </div>
                                    </a>
                                    <a href="{{ route('blog.index', $post->category_id) }}" class="blog-post-permalink">
                                        <p class="post-category">Category: {{ $post->category->title }}</p>
                                    </a>
                                    <a href="{{ route('blog.show', $post->id) }}" class="blog-post-permalink">
